...
test "address" {
    ...
    defer area.deinit();
    const payload = (try engine.parseHexBytes(area.allocator(), "162c5ea71c0b23f5b9022ef047c4a86470a5b070"))[0..20];
    ...
    const hexAddress = try engine.hexAlloc(area.allocator(), pubkey.asBytes());
    try std.testing.expectEqualSlices(u8, hexAddress, "76a914162c5ea71c0b23f5b9022ef047c4a86470a5b07088ac");
...
test "p2ppkh address base58" {
    ...
    try std.testing.expectEqualSlices(u8, try engine.hexAlloc(area.allocator(), script.asBytes()), "76a914162c5ea71c0b23f5b9022ef047c4a86470a5b07088ac");
    try std.testing.expectEqualSlices(u8, try address.toString(area.allocator()), "132F25rTsvBdp9JzLLBHP5mvGY66i1xdiM");
    try std.testing.expectEqual(address.addressType().?, .P2pkh);
}
...
test "p2pkh from key" {
    ...
    defer area.deinit();
    {
        const key = try PublicKey.fromSlice(try parseHex(area.allocator(), "048d5141948c1702e8c95f438815794b87f706a8d4cd2bffad1dc1570971032c9b6042a0431ded2478b5c9cf2d81c124a5e57347a3c63ef0e7716cf54d613ba183"));
    ...
    {
        const key = try PublicKey.fromSlice(try parseHex(area.allocator(), "03df154ebfcf29d29cc10d5c2565018bce2d9edbab267c31d2caf44a63056cf99f"));
...
test "p2sh address base58" {
    var area = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer area.deinit();
    const payload = try engine.parseHexBytes(area.allocator(), "162c5ea71c0b23f5b9022ef047c4a86470a5b070");
    ...
    const script = address.scriptPubKey(area.allocator());
    try std.testing.expectEqualSlices(u8, try engine.hexAlloc(area.allocator(), script.asBytes()), "a914162c5ea71c0b23f5b9022ef047c4a86470a5b07087");
...
test "p2sh parse" {
    var area = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer area.deinit();
    const payload = try parseHex(area.allocator(), "");
...
test "p2wpkh" {
    var area = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer area.deinit();
    var key = try PublicKey.fromSlice(try parseHex(area.allocator(), "033bc8c83c52df5712229a2f72206d90192366c36428cb0c12b6af98324d97bfbc"));
...
test "p2wsh" {
    ...
    var area = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer area.deinit();
    const script = try Script.fromBytes(area.allocator(), try parseHex(area.allocator(), "52210375e00eb72e29da82b89367947f29ef34afb75e8654f6ea368e0acdfd92976b7c2103a1b26313f430c4b15bb1fdce663207659d8cac749a0e53d70eff01874496feff2103c96d495bfdd5ba4145e3e046fee45e84a8a48ad05bd8dbb395c011a32cf9f88053ae"));
...
test "p2shwpkh" {
    ...
    var area = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer area.deinit();
    var key = try PublicKey.fromSlice(try parseHex(area.allocator(), "026c468be64d22761c30cd2f12cbc7de255d592d7904b1bab07236897cc4c2e766"));
...
test "p2shwsh" {
    ...
    var area = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer area.deinit();
    const script = try Script.fromBytes(area.allocator(), try parseHex(area.allocator(), "522103e5529d8eaa3d559903adb2e881eb06c86ac2574ffa503c45f4e942e2a693b33e2102e5f10fcdcdbab211e0af6a481f5532536ec61a5fdbf7183770cf8680fe729d8152ae"));
...
test "non existent segwit version" {
    ...
    const version: u5 = 13;
    // 40-byte program
    const program = try parseHex(area.allocator(), "654f6ea368e0acdfd92976b7c2103a1b26313f430654f6ea368e0acdfd92976b7c2103a1b26313f4");
    const address = Address{
        .network = .bitcoin,
...
test "bip173 vectors" {
    ...
    for (vectors) |vector| {
        ...
        try std.testing.expectEqualSlices(u8, try engine.hexAlloc(area.allocator(), address.scriptPubKey(area.allocator()).asBytes()), vector[1]);
    }
    ...
    for (invalidVectors) |vector| {
        _ = Address.fromString(area.allocator(), vector) catch {
            continue;
        };
        unreachable;
    }
