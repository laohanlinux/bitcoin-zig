...
test "address" {
    ...
    const hexAddress =
        (try engine.hex(pubkey.asBytes()))[0..];
...
test "p2ppkh address base58" {
    ...
    try std.testing.expectEqualSlices(u8, try engine.hex(script.asBytes()), "76a914162c5ea71c0b23f5b9022ef047c4a86470a5b07088ac");
    try std.testing.expectEqualSlices(u8, try address.toString(area.allocator()), "132F25rTsvBdp9JzLLBHP5mvGY66i1xdiM");
    try std.testing.expectEqual(address.addressType().?, .P2pkh);
}
...
test "p2sh address base58" {
    ...
    const script = address.scriptPubKey(area.allocator());
    try std.testing.expectEqualSlices(u8, try engine.hex(script.asBytes()), "a914162c5ea71c0b23f5b9022ef047c4a86470a5b07087");
...
test "bip173 vectors" {
    ...
    for (vectors) |vector| {
        ...
        try std.testing.expectEqualSlices(u8, try engine.hex(address.scriptPubKey(area.allocator()).asBytes()), vector[1]);
    }
